...
@SpringBootTest
class UPIServicesTransactionImplTest {
	...
	@Autowired
	IUPITransactionServices upiTransactionServices;
	...
	@Test
	void testProcessUPITransaction() throws LowBalanceException, InvalidUPIDetailsException {
		when(upiRepository.findByUpiId(newTransactionByUpi.getUpiId())).thenReturn(upi.get());
		assertEquals(true,upiTransactionServices.processUPITransaction(newTransactionByUpi));
	
		
	}

	@Test
	void testVerifyUPIDetails() throws InvalidUPIDetailsException {
		when(upiRepository.findByUpiId(newTransactionByUpi.getUpiId())).thenReturn(upi.get());
		when(transactionRepositoryForUpi.findByTransactionIdForPaymentByUpi("1000")).thenReturn(transactionByUpi);
		assertEquals(true, upiTransactionServices.verifyUPIDetails(newTransactionByUpi));
	}
